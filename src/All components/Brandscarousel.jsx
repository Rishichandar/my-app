...
//     return (
//         <div className="w-full px-4 py-8 mx-auto relative"> {/* Center the carousel and adjust width */}
//             {Object.keys(brandRows).map((rowKey, index) => (
//                 brandRows[rowKey].length > 0 && (
//                     <div key={index} className="mb-8"> {/* Margin between rows */}
...
const Brandscarousel = () => {
    ...
    useEffect(() => {
        fetch('/combineddata.json') // Update to fetch from combineddata.json
            .then((response) => response.json())
            .then((data) => setBrandRows(data.brands)) // Extract brands from the data
            .catch((error) => console.error('Error fetching brands:', error));
    }, []);
